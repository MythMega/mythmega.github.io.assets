...
PAGES = {
    "sts1": {
        "url": "https://slaythespire.wiki.gg/wiki/Potions_List",
        "img_dir": "../../assets/category/sts/potions/",
        "json_file": "sts1_potions.json"
    },
    "sts2": {
        "url": "https://slaythespire.wiki.gg/wiki/Slay_the_Spire_2:Potions_List",
        "img_dir": "../../assets/category/sts2/potions/",
        "json_file": "sts2_potions.json"
    }
}
...
def scrape_page(cfg):
    ...
    while waited < max_wait:
        try:
            boxes = driver.find_elements(By.CSS_SELECTOR, "div.potion-box")
            if boxes:
                break
        except Exception:
            pass
    ...
    for box in soup.select("div.potion-box"):
        title_tag = box.select_one(".potion-title")
        if not title_tag:
            continue
        name = title_tag.text.strip()

        img_tag = box.select_one(".potion-image-wrap img")
        img_url = None
        if img_tag:
    ...
    with open(json_file, "w", encoding="utf-8") as f:
        json.dump(potions, f, indent=4, ensure_ascii=False)

    if failures:
        with open("potions_download_failures.log", "a", encoding="utf-8") as lf:
            for fail in failures:
                lf.write(json.dumps(fail, ensure_ascii=False) + "\n")

    return len(potions), len(failures)

if __name__ == "__main__":
    for key, cfg in PAGES.items():
        print(f"Scraping {key} …")
        count, fails = scrape_page(cfg)
        print(f"{count} potions trouvées, {fails} échecs d'image → {cfg['json_file']}")
